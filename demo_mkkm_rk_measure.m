function demo_mkkm_rk_measure(n, nRepeat)
addpath('./MKKM_RK/');
addpath('./lib');
...
measurements={'sfn','fip','ld'}; % {'sfn','fip','vn','ld'};
dataset=datasets{n};
...
result_dir = fullfile(pwd,['result_mkkm_rk_measure' num2str(nRepeat)],[dataset,'_result']);
...
for i = 1:length(lambdas)
    ...
    mkkm_rk_result_mean=zeros(length(measurements),3);
    ...
    for m=1:length(measurements)
        ...
        for iRepeat = 1:nRepeat
            ...
            [label,kw,obj,Y] = MKKM_RK(Y,nClass,1e-5,Ks,measurement,lambda);
        ...
        if size(mkkm_rk_result,1) > 1
            mkkm_rk_result_mean(m,:) = mean(mkkm_rk_result);
        else
            mkkm_rk_result_mean(m,:) = mkkm_rk_result;
        end
    end
